...
bool ofxHairModel::loadHairModel(string filename)
{
	...
	for(int i=0; i<strands.size(); i++){
		...
		for(int j=0; j<strands[i].m_resolution; j++){
			ofxHairParticle p(positions[count], 1.0);
...
bool ofxHairModel::loadGuideHair(string filename)
{
	...
	if (ofs.fail()) {
        std::cerr << "failed loading guide hair" << std::endl;
		return false;
    }
	cout << "readed" << endl;
	...
	mGuideJoints.resize(num*100);
	cout << "joints num : " << mGuideJoints.size() << endl;

	while (getline(ofs, str)) {
		int guideIndex = ofToInt(str);
		//cout << guideIndex << "/" << strands.size() << endl;
		strands[guideIndex].bGuideHair = true;
    }

	mNormalVertices.resize(m_numParticles - mGuideJoints.size());

	int k = 0;
	int l = 0;
	for(int i=0; i<strands.size(); i++){
		ofColor c;
		if(strands[i].bGuideHair){
			for(int j=0; j<strands[i].m_resolution; j++){
				mGuideJoints[100 * k + j].mRotate = strands[i].m_particles[j].gMat.getRotate();
				mGuideJoints[100 * k + j].mTranslation = strands[i].m_particles[j].gMat.getTranslation();
				mGuideJoints[100 * k + j].mPos = strands[i].m_particles[j].position;
				//cout << mGuideJoints[100 * k + j].mPos << endl;
				//cout <<  << endl;
			}
			k++;
		}
			...
			c.setHsb(ofRandom(0, 255), 200, 255, 255);
			for(int j=0; j<strands[i].m_resolution; j++){
				mNormalVertices[100 * l + j].mPosition = strands[i].m_particles[j].position;
				mNormalVertices[100 * l + j].mColor = ofVec3f((float)c.r / 255.f, (float)c.g / 255.f, (float)c.b / 255.f);
				mNormalVertices[100 * l + j].mVertIndex = 100 * l + j;
				if(j!=0)
					strands[i].m_particles[j].enabled = false;
			}
			l++;
		}
	}

	cout << "finished loading guide hair" << endl;
...
void ofxHairModel::resetGuideAndNormal()
{
	for (auto& s : strands)
	{
		if (!s.bGuideHair) {
...
bool ofxHairModel::exportGuideHairModel(string filename)
{
	std::cout << "exporting... " << std::endl;
	...
	// int : total strand size
	int numGStrands = mGuideJoints.size() / strands[0].getResolution();
	fout.write((char*)&numGStrands, sizeof(int));
